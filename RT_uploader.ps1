...
Write-Host ( 'Найдено архивов: {0}, требующих переноса на гугл-диск, начинаем!' -f $sum_cnt)
# Перебираем архивы.
foreach ( $zip in $zip_list ) {
    ...
    Write-Host ''
    Write-Host ( 'Начинаем перенос раздачи {0}, {1} ({2})' -f $torrent_id, $torrent.name, (Get-FileSize $zip.Size) ) -ForegroundColor Blue
    ...
    if ( Test-Path -Path $zip_google_path ) {
        Write-Host 'Такой архив уже есть на гугл-диске, пропускаем.'
        Continue
    }
