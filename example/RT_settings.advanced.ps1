...
# Настроки для collector или restorator
$collector = @{
    # каталог, в который скачивать раздачи
    collect      = '/data/downloads'
    # условный общий объём раздач, которые влезут в каталог выше.
...
# Настройки чисто под restorator
$restorator = @{
	# каталог для восстановления из архивов
	path = 'K:\Хранимое\Прочее'
	# количество дней, за которые подбирать запросы на восстановление
	look_behind_days = 1
	# количество дней сидирования после восстановления
	keep_seeding_days = 3
	# Категория для восстанавливаемых раздач
	category = 'Restored'
	# максимальный размер восстанавливаемых раздач
	max_size = 10 * [math]::Pow(1024, 3) # 10 гигабайт, например
}
